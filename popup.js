...
function readSettings() {
  return {
    ...
    typingDelay: Number(document.getElementById("typingDelay").value) || 70
  };
}
...
async function fillCurrentTab() {
  ...
  try {
    ...
    if (!response?.ok) {
      setStatus(`Autofill failed: ${response?.error || "Unknown error"}`, true);
      return;
    }

    setStatus(`Autofill done. Fields filled: ${response.filled}.`);
  } catch (error) {
    setStatus(
      "Could not connect to page. Open booking page first, then try again.",
      true
    );
  }
}
